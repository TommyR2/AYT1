(() => {
  ...
  function extractCeremonyFromWeeklyData(weeklyData) {
    ...
    const meta = { beams: c.beams ?? weeklyData.beams, blackout: c.blackout ?? weeklyData.blackout, week: weeklyData.week };
    return { pairs, meta };
  }
  ...
  async function loadCeremonyForWeek(week, weeklyData) {
    ...
    if (extra) {
      ...
      const meta = { beams: c.beams, blackout: c.blackout, week: c.week ?? week };
  ...
  function buildSceneFor(data) {
    ...
    // Legend gradient
    const legendWidth = 260, legendHeight = 28;
  ...
  async function selectWeek(w) {
    ...
    renderCeremonyTable(w, pairs, meta, priorSet);
